...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		if (constructorToUse == null) {
			...
			if (explicitArgs != null) {
				...
				// 从 BeanDefiniton 中获取构造参数，也就是从配置文件中提取构造参数
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (explicitArgs != null) {
			...
			// 缓存中存在，则解析存储在 BeanDefiniton 中的参数
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			for (Method candidate : rawCandidates) {
				// 如果有static 且为工厂方法, 则添加到 candidateSet中
				if (Modifier.isStatic(candidate.getModifiers()) == isStatic && mbd.isFactoryMethod(candidate)) {
			...
			if (explicitArgs != null) {
				...
				// getBean() 没有传递参数，则需要解析保存在 BeanDefiniton 构造函数中指定的参数
				...
				if (mbd.hasConstructorArgumentValues()) {
					...
					ConstructorArgumentValues cargs = mbd.getConstructorArgumentValues();
					// 解析构造函数的参数
					// 将改 bean 的构造函数是参数解析为 resolvedValues 对象，其中会涉及到其他bean
					resolvedValues = new ConstructorArgumentValues();
					minNrOfArgs = resolveConstructorArguments(beanName, mbd, bw, cargs, resolvedValues);
				}
				else {
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						try {
							...
							// PaarameterNameDiscoverer 是用于解析方法和构造函数的参数名称的接口，为参数名称探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							if (pnd != null) {
								// 获取指定构造函数的参数名称
								paramNames = pnd.getParameterNames(candidate);
					...
					// 但是，仅在非宽松构造函数解析模式下执行该检查，并显示忽略重写方法（具有相同的参数签名）
